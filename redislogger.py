...
class RedisLogger(logging.Handler):
  def __init__(self, host, list_name, servicename, port=6379, db=0):
  ...
  def emit(self, record):
    try:
      msg = simplejson.dumps({'service':self.servicename, 'logline':self.format(record)})
      self.r.lpush(self.list_name, msg)
    except:
      try:
